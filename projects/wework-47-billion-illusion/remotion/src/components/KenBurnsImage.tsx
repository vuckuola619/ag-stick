...
export const KenBurnsImage: React.FC<KenBurnsImageProps> = (props) => {
  const {
    imagePath,
    imageSrc,
    videoPath,
  ...
  const getResolvedSrc = (src?: string) => {
    ...
    let relativePart = cleanPath;
    if (cleanPath.includes('/public/')) {
      relativePart = cleanPath.split('/public/')[1];
    } else if (cleanPath.includes('/images/')) {
      relativePart = 'assets/images/' + cleanPath.split('/images/')[1];
    } else if (cleanPath.includes('/audio/')) {
      relativePart = 'assets/audio/' + cleanPath.split('/audio/')[1];
    } else if (cleanPath.startsWith('/')) {
      relativePart = cleanPath.slice(1);
    }
    try {
  ...
  const rawSrc = videoPath || videoSrc || imagePath || imageSrc;
