...
RAW_VIDEO = os.path.join(EXPORT_DIR, "demon_core_v2_raw.mp4")
FINAL_MASTER = os.path.join(EXPORT_DIR, "the_demon_core_v2_10min_master.mp4")

os.makedirs(EXPORT_DIR, exist_ok=True)

def render_demon_core_v2():
    print("=======================================================================", flush=True)
    print("=== [1/2] Rendering Remotion Composition 'DemonCoreV2'              ===", flush=True)
    print("===       (18,252 frames @ 30fps | Duration: ~10:07)                 ===", flush=True)
    print("=== Specs: 1920x1080 Widescreen, 32 Scenes, 100% 2D Comic Neon Rush ===", flush=True)
...
def mux_broadcast_audio():
    ...
    print(f"\n[SUCCESS] Final 10-Minute Documentary Master V2 Generated!", flush=True)
    print(f"  File : {FINAL_MASTER}", flush=True)
    print(f"  Size : {size_mb:.2f} MB", flush=True)

def main():
    render_demon_core_v2()
    mux_broadcast_audio()
